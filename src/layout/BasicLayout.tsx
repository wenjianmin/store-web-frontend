    ...
    return routers?.map(item => {
      if (item?.children) {
        ...
        return {
          ...
          hideInMenu: item?.hideInMenu || !item?.children?.find(citem => authMenus?.includes(citem?.name))
        }
      }
      return {
        ...item,
          ...
          menuFooterRender={props => {
            return (
              ...
              >
                <div>
                  作者：
                  <a href='https://juejin.cn/user/1433418891795806' rel="noreferrer" target='_blank'>「寻找奶酪的mouse」</a>
                </div>
              </div>
            )
          }}
